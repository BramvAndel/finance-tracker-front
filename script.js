...
function initializeModals() {
  ...
  document
    ...
    .addEventListener("click", openUserProfileModal);
...
async function openEditExpenseModal(expenseId) {
  ...
  document.getElementById("editExpenseStoreName").value =
    expense.store_name || "";
...
async function handleEditExpense(e) {
  ...
  const expenseData = {
    ...
    description:
      document.getElementById("editExpenseDescription").value || null,
  };
...
function populateCategorySelects() {
  ...
  selects.forEach((select) => {
    ...
    const options = categories
      .map((cat) => `<option value="${cat.category_id}">${cat.name}</option>`)
      .join("");
...
function displayCategories(categoriesToShow) {
  ...
  tbody.innerHTML = categoriesToShow
    .map(
      (category) => `
        <tr>
...
function filterMyCategories() {
  const search = document
    .getElementById("myCategorySearchFilter")
    .value.toLowerCase();
...
function displayRecentExpenses(recentExpenses) {
  ...
  if (recentExpenses.length === 0) {
    container.innerHTML =
      '<p style="text-align: center; color: var(--text-light); padding: 2rem;">No expenses yet</p>';
    return;
  }
...
function initializeFilters() {
  // Expense filters
  document
    .getElementById("expenseSearchFilter")
    ?.addEventListener("input", filterExpenses);
  document
    .getElementById("expenseCategoryFilter")
    ?.addEventListener("change", filterExpenses);
  document
    .getElementById("expenseDateFromFilter")
    ?.addEventListener("change", filterExpenses);
  document
    .getElementById("expenseDateToFilter")
    ?.addEventListener("change", filterExpenses);
  document
    .getElementById("clearExpenseFilters")
    ?.addEventListener("click", clearExpenseFilters);
  ...
  document
    .getElementById("clearUserFilters")
    ?.addEventListener("click", clearUserFilters);

  // Category filters
  document
    .getElementById("categorySearchFilter")
    ?.addEventListener("input", filterCategories);
  document
    .getElementById("clearCategoryFilters")
    ?.addEventListener("click", clearCategoryFilters);

  // My Category filters
  document
    .getElementById("myCategorySearchFilter")
    ?.addEventListener("input", filterMyCategories);
  document
    .getElementById("clearMyCategoryFilters")
    ?.addEventListener("click", clearMyCategoryFilters);
}
...
function displayExpenseDetails(expense) {
  ...
  document.getElementById("detailExpenseAmount").textContent =
    `$${parseFloat(expense.amount).toFixed(2)}`;
  document.getElementById("detailExpenseCategory").textContent =
    expense.categories || "Uncategorized";
  document.getElementById("detailExpenseDate").textContent = formatDate(
    expense.expense_date,
  );
  document.getElementById("detailExpenseStoreName").textContent =
    expense.store_name || "Not specified";
  document.getElementById("detailExpenseDescription").textContent =
  ...
  if (expense.first_name && expense.last_name) {
    document.getElementById("detailExpenseUser").textContent =
